...
const StylesheetEditorShell = lazy(
	() => import("@/features/resume/stylesheet/editor"),
);

export function CustomStylesSectionBuilder() {
	const collapsed = useSectionStore(
		(state) => state.sections.styles?.collapsed ?? false,
	);
			...
			{collapsed ? null : (
				<Suspense
					fallback={
						<div
							role="status"
							className="h-72 animate-pulse rounded-md bg-muted"
